            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        for (newNode = _nodes.begin(); newNode != _nodes.end(); newNode++){
                            if ((*newNode)->GetID() == id){ break; }
                        }
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            for (parentNode = _nodes.begin(); parentNode != _nodes.end(); parentNode++){
                                if ((*parentNode)->GetID() == std::stoi(parentToken->second)){ break; }
                            }

                            std::vector<std::unique_ptr<GraphNode>>::iterator childNode;
                            for (childNode = _nodes.begin(); childNode != _nodes.end(); childNode++){
                                if ((*childNode)->GetID() == std::stoi(childToken->second)){ break; }
                            }
                            ...
                            std::unique_ptr<GraphEdge> uedge(edge);
                            // (*parentNode)->AddEdgeToChildNode(std::move(edge));
                            (*parentNode)->AddEdgeToChildNode(std::move(uedge));
                        }
